							...
							<thead>
								<tr>
									<th style="width:8px;"><input type="checkbox"
										...
										data-set="#table_ShowDevice .checkboxes" />
									</th>
							...
							<tbody>
								<c:forEach items="${devicelist}" var="device">
									<tr class="odd gradeX">
										...
										<td class="hidden-480"><fmt:formatDate type="date"
												value="${device.produceDate}" dateStyle="default" />
										</td>
										<td class="hidden-480">${device.amount}</td>
										<td class="hidden-480">${device.unit}</td>
										<td class="hidden-480">${device.substationNo}</td>
										<td class="hidden-480"><fmt:formatDate type="date"
												value="${device.firstRunDate}" dateStyle="default" />
										</td>
										<td class="hidden-480"><fmt:formatDate type="date"
												value="${device.secondRunDate}" dateStyle="default" />
										</td>
										<td class="hidden-480"><fmt:formatDate type="date"
												value="${device.invalidatedDate}" dateStyle="default" />
										</td>
                                    	...
                                    	<td class="hidden-480">
                                    		<a href="findParameterDetails.action?deviceType=${device.deviceType}&deviceModel=${device.deviceModel}" class="btn mini blue" target="rightFrame">技术参数详情</a>
                                    		<c:if test="${user == 'employee'}">
										 	...
										 	<c:if test="${user == 'manager'}">
												...
												<c:if test="${device.deviceStatus == 0 or device.deviceStatus == 4 or device.deviceStatus == 8}">
												 	<a href="javascript:if(confirm('确定删除吗?'))window.location='deleteDevice.action?deviceId=${device.deviceId}'" class="btn mini gray" target="rightFrame"><i class="icon-trash"></i> 删除</a>
												</c:if>
											</c:if>
										</td>
									</tr>
